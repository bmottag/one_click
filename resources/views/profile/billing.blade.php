			...
			<div id="kt_app_content_container" class="app-container container-fluid">
				...
				<div id="kt_user_profile_nav" class="rounded bg-gray-200 d-flex flex-stack flex-wrap mb-9 p-2">
					...
					<ul class="nav flex-wrap border-transparent">
						...
						<!--begin::Nav item-->
						<li class="nav-item my-1">
							<a class="btn btn-sm btn-color-gray-600 bg-state-body btn-active-color-gray-800 fw-bolder fw-bold fs-6 fs-lg-base nav-link px-3 px-lg-4 mx-1 active" href="{{ route('profile.billing') }}">Billing</a>
						</li>
						<!--end::Nav item-->
					</ul>
					<!--end::Nav-->
				...
				<div class="card mb-5 mb-xl-10">
					...
					<div class="card-body">
						...
						@if(session('success') || session('error'))
							<div class="notice d-flex rounded border border-dashed mb-12 p-6
								{{ session('success') ? 'bg-light-success border-success' : 'bg-light-warning border-warning' }}">
								<!--begin::Icon-->
								<span class="svg-icon svg-icon-2tx me-4
									{{ session('success') ? 'svg-icon-success' : 'svg-icon-warning' }}">
						...
						<div class="row">
							...
							<div class="col-lg-7">
								@if($user->role === 'registered_user')
									...
									<h3 class="mb-2">You have a Free Account</h3>
									<p class="fs-6 text-gray-600 fw-semibold mb-6 mb-lg-15">
										Upgrade to unlock premium features and extended usage.
									</p>
								@else
									@if($lastSubscription)
										@if($lastSubscription->ends_at && $lastSubscription->ends_at->isFuture())
											<h3 class="mb-2">
												Active until {{ $lastSubscription->ends_at->format('M d, Y') }}
											</h3>
											<p class="fs-6 text-gray-600 fw-semibold mb-6 mb-lg-15">
												We will send you a notification upon Subscription expiration.
											</p>
										@elseif($lastSubscription->ends_at && $lastSubscription->ends_at->isPast())
											<h3 class="mb-2">
												Expired on {{ $lastSubscription->ends_at->format('M d, Y') }}
											</h3>
											<p class="fs-6 text-gray-600 fw-semibold mb-6 mb-lg-15">
												Your subscription has ended. Please renew to continue enjoying premium features.
											</p>
										@else
							...
							<div class="col-lg-5">
								@if($user->role === 'registered_user')
									<!-- Mostrar solo botón de suscripción -->
									...
									@if($lastSubscription)
										<div class="fs-5 mb-2">
											<span class="text-gray-800 fw-bold me-1">
												${{ number_format($lastSubscription->amount / 100, 2) }}
											</span>
											<span class="text-gray-600 fw-semibold">
												Per {{ ucfirst($lastSubscription->interval) }}
											</span>
										</div>
								...
								<table class="table table-row-bordered align-middle gy-4 gs-9">
									<thead class="border-bottom border-gray-200 fs-6 text-gray-600 fw-bold bg-light bg-opacity-75">
										<tr>
											...
											<td class="min-w-150px">Ends At</td>
										</tr>
									</thead>
									<tbody class="fw-semibold text-gray-600">
										@forelse($subscriptions as $subscription)
											<tr>
												...
												<td>
													<a href="#">
														{{ ucfirst($subscription->plan_id) }} plan ({{ $subscription->interval }})
													</a>
												</td>
												<td>${{ number_format($subscription->amount / 100, 2) }}</td>
												<td class="text-right">
													<span class="badge {{ $subscription->payment_status === 'paid' ? 'badge-success' : 'badge-danger' }}">
														{{ ucfirst($subscription->payment_status) }}
													</span>
												</td>
												<td>{{ optional($subscription->ends_at)->format('M d, Y') ?? 'N/A' }}</td>
											</tr>
										@empty
											<tr>
